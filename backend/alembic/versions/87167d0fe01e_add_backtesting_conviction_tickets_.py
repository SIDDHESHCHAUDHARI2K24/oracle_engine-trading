...
def upgrade() -> None:
    ...
    """)
    op.create_index(
        "idx_metrics_run_passed",
        "backtest_metrics",
        ["backtest_run_id", "passed"],
        unique=False,
    )
    op.create_index(
        "idx_metrics_ticker_strategy_time",
        "backtest_metrics",
        ["ticker_id", "strategy_name", "computed_at"],
        unique=False,
    )
    op.create_table(
        "conviction_tickets",
        ...
        sa.Column(
            "backtest_pass_strategies", postgresql.ARRAY(sa.String()), nullable=False
        ),
        sa.Column("status", sa.String(length=20), nullable=False),
    ...
    op.create_index(
        "idx_tickets_inbox",
        "conviction_tickets",
        ["universe_id", "status", "conviction_score"],
        unique=False,
    )
    op.create_index(
        "idx_tickets_resolution",
        "conviction_tickets",
        ["resolution_date", "status"],
        unique=False,
    )
    op.create_index(
        "idx_tickets_ticker_date",
        "conviction_tickets",
        ["ticker_id", "inference_date"],
        unique=False,
    )
    op.create_table(
        "filter_runs",
        ...
        sa.Column(
            "filter_config", postgresql.JSONB(astext_type=sa.Text()), nullable=False
        ),
        sa.Column("metadata", postgresql.JSONB(astext_type=sa.Text()), nullable=False),
        sa.Column(
            "id", sa.UUID(), server_default=sa.text("gen_random_uuid()"), nullable=False
