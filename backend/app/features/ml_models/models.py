...
class TrainingRun(Base, UUIDPrimaryKey):
    ...
    universe_id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True), sa.ForeignKey("universes.id", ondelete="CASCADE"), nullable=False, index=True
    )
    triggered_by: Mapped[str] = mapped_column(nullable=False)
    started_at: Mapped[datetime] = mapped_column(
        DateTime(timezone=True),
        nullable=False, default=_utc_now, server_default=func.now()
    )
    ...
    model_metadata: Mapped[dict] = mapped_column(
        "metadata", JSONB, nullable=False, default=dict, server_default=func.jsonb_build_object()
    )
...
class ModelArtifact(Base, UUIDPrimaryKey):
    ...
    universe_id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True), sa.ForeignKey("universes.id", ondelete="CASCADE"), nullable=False
    )
    training_run_id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True), sa.ForeignKey("training_runs.id", ondelete="CASCADE"), nullable=False
    )
...
class Prediction(Base):
    ...
    inference_run_id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True), sa.ForeignKey("inference_runs.id", ondelete="CASCADE"), nullable=False
    )
    ...
    lstm_outputs: Mapped[list] = mapped_column(
        ARRAY(Float), nullable=False
    )
    tft_q10: Mapped[list] = mapped_column(
        ARRAY(Float), nullable=False
    )
    tft_q50: Mapped[list] = mapped_column(
        ARRAY(Float), nullable=False
    )
    tft_q90: Mapped[list] = mapped_column(
        ARRAY(Float), nullable=False
    )
