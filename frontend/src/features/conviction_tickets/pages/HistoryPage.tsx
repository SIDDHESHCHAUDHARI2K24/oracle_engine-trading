...
const STATUSES = ['', 'TRADABLE', 'REVIEWED', 'ACTIONED', 'RESOLVED', 'EXPIRED']
const OUTCOMES = ['', 'WIN', 'LOSS', 'FLAT']
...
export function HistoryPage(): JSX.Element {
  ...
  const [sorting, setSorting] = useState<SortingState>([
    { id: 'inference_date', desc: true },
  ])

  const { data, isLoading, isError, error, refetch } = useTicketHistory(
  ...
  const columns = useMemo(
    () => [
      ...
      columnHelper.accessor('inference_date', {
        header: 'Date',
        cell: (info) => (
      ...
      columnHelper.accessor('actual_return', {
        header: 'Actual Ret',
        cell: (info) => {
          const val = info.getValue()
          if (val === null) return <span className="text-sm text-muted-foreground">\u2014</span>
          return (
            <span
    ...
    <main className="mx-auto max-w-6xl p-8">
      <div className="mb-6">
        ...
        <p className="text-muted-foreground mt-1">
          Historical conviction ticket records with outcomes.
        </p>
      </div>
        ...
        <Card>
          <CardContent className="py-12 text-center">
            <p className="text-red-600" role="alert">
              {error instanceof Error ? error.message : 'Failed to load ticket history'}
            </p>
      ...
      {data && !isLoading && !isError && data.tickets.length === 0 && (
        <Card>
          <CardContent className="py-12 text-center">
            <p className="text-muted-foreground">No ticket history found.</p>
          </CardContent>
        </Card>
      )}
...
function OutcomeBadge({ outcome }: { readonly outcome: string }): JSX.Element {
  ...
  return (
    <span
      className={`inline-flex items-center rounded-full px-2 py-0.5 text-xs font-medium ${colors[outcome] ?? 'bg-gray-100 text-gray-800'}`}
    >
      {outcome}
    </span>
  )
